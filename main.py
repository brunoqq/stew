...
async def on_ready():
    ...
    print("=================================")
    await client.change_presence(game=discord.Game(name='no jogar.stewnetwork.com.br'), status=discord.Status.dnd)
...
##DEFAULT_COMMANDS
    ...
    if message.content.lower().startswith('!comandos'):
        embed = discord.Embed(
            ...
            description="!serverinfo » Veja as informações do servidor\n"
                        ...
                        "!ajuda » Veja as informações básicas do servidor StewNetwork\n"
    ...
    if message.content.lower().startswith('!ajuda'):
        embed = discord.Embed(
            title="Você necessita de nossa ajuda? Espero te ajudar! Abaixo terá todas as informações necessarias para você!",
            color=0x0d488a,
            description="**Seja bem-vindo ao discord da rede StewNetwork! Segue abaixo algumas informações da rede que possa te ajudar!**\n"
                        "\n"
                        "IP: jogar.stewnetwork.com.br\n"
                        "\n"
                        "Site: [clique aqui!](http://stewnetwork.com.br/)\n"
                        "\n"
                        "Twitter da rede: [clique aqui!](https://www.twitter.com/StewServidores)\n"
                        "\n"
                        "Loja: **Em breve!**\n"
                        "\n"
                        "Fórum: **Em breve!**\n"
                        "\n"
                        "Para ver alguns comandos meus, digite `!comandos` em nosso chat de comandos!\n"
                        "\n"
                        "__**Caso precise de outro tipo de ajuda contate um membro da equipe!**__"
        )
        embed.set_author(
            name="StewNetwork",
            icon_url=client.user.avatar_url
        ...
        await client.send_message(message.channel, "Olá {}, foi enviado algumas informações da nossa rede em seu privado!".format(
            message.author.mention))
        await client.send_message(message.author, embed=embed)
    #avatar
    ...
    if message.content.lower().startswith('!convite'):
     ...
     await client.send_message(message.author, "Seu convite para o discord do StewNetwork é: {}".format(invite.url))
     await client.send_message(message.channel, "Olá {}, um convite temporário foi enviado no seu privado!".format(message.author.mention))
##STAFF+
    #staff+
...
async def on_member_join(member):
      ...
      channel = client.get_channel('425675735693524996')
      serverchannel = member.server.default_channel
      embedmsg = discord.Embed(
          title="Olá {}!".format(member.name),
          description="Bem vindo ao discord da rede de servidores StewNetwork!",
          color=0x0d488a,
      )
